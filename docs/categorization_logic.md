...
| 5  | `rule_short_garbage`   | *(skipped entirely if the line is `forgiven`, if `is_structured_line()` holds, or if `is_domain_notation()` holds — see `rule_domain_notation` below)* — `word_count ≤ ISOLATED_CHAR_MIN_TOKENS` (def: 3) **and** no Czech 🇨🇿 diacritics **and** (`lang_score ≤ LANG_SCORE_REMAP` (def: 0.75) **or** `rot_ratio ≥ SUSPICIOUS_ROT_RATIO` (def: 0.65)) **and** (gibberish present **or** `word_weird > 0`) | `Trash`                                                               | Structural short-garbage route (e.g. `olie`). Recorded as `trash_threshold`. Returns unconditionally — it does **not** consult `check_rescues()`, and it is the **only** structural route into `Trash` that convicts without a second witness; its six §9 neighbours all require `_has_strong_garbage_evidence()`. Issue #30 is about that asymmetry, and about the fact that on the short diacritic-free population the predicate returns `False` for *everything* — garbage and real domain vocabulary alike — so requiring it here is not a narrowing of this rule but a suspension of it. `_has_shape_garbage_evidence()` (`SHORT_GARBAGE_WITNESS_ENABLE`, **off by default**) is the second witness that does discriminate; it has no call site yet and is documented under §5c below.                                                                                                                                                                                                                                                                                                |
...
| 5c | `_has_shape_garbage_evidence()` *(no rule name yet)* | **Off by default** (`SHORT_GARBAGE_WITNESS_ENABLE=false`). A phonotactic **shape** test over sub-tokens of ≥ `SHORT_GARBAGE_WITNESS_MIN_ALPHA` (def: 4) alphabetic characters: a 3+ vowel run, the same character three times in a token of ≤ 8 letters, a doubled consonant in first position, or fewer than `SHORT_GARBAGE_WITNESS_VARIETY_MAX` (def: 0.50) distinct letters at ≥ 7 letters. Vetoed by `has_cz_diacs()`, `is_structured_line()` and `is_domain_notation()`, and by the `_NEUTRAL_LEXICON` / `SHORT_EXCEPTION_TOKENS` / `SHORT_VALID_WORDS` sets. | *(no category of its own; intended as a second disjunct for gate 5)* | Reaches `oueussd`, `sektlll`, `cuxoaid`, `Tthts I`, `rragment`, `NINNNIC`, `vansasaasasa` — the garbage `_has_strong_garbage_evidence()` misses — while leaving `malakofauna`, `diapozitiv`, `Equus caballus`, `Skelettmaterial` and `vrstva` alone. It reuses `_RE_FUSED_VOWEL_RUN` but deliberately **not** `detect_fused_words()`'s other two clauses: `len(core) > 14` flags `Skelettmaterial`, and the 5+ consonant run flags `vrstva`, `ctvrtek`, `zmrzl` — ordinary diacritic-free Czech. **Known limit:** the phonotactically legal residue (`edelite`, `vfetennl k.`) is out of reach by construction and needs a lexicon (#30 D14). Do not enable before measuring against annotated lines — the in-tree sample corpus is 15 lines. |
